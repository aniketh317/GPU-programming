...
void executeTicketingSystemParticipation(int ticketNumber) {
	...
	waitForTicketNumber.wait(lock, [ticketNumber] { return (ticketNumber == currentTicketNumber); });
	...
	currentTicketNumber++;
	waitForTicketNumber.notify_all();
}
...
int manageTicketingSystem() {
	...
	std::unique_lock<std::mutex> lock(sharedMutex);
	waitForTicketNumber.wait(lock, []{return (currentNumThreads == currentTicketNumber);});
