...
__host__ void retrieveDataFromFiles(int *h_a, int *h_b, int numElements, int i)
{
    ...
    bool iterProduce = false; //Are values for the current iteration produced
...
__host__ void performMultiCPUIteration(int i)
{
    int numElements = 128;
    ...
    retrieveDataFromFiles(h_a, h_b, numElements, i);
    
    // Copy data from host to the device 
    auto[d_a, d_b] = allocateDeviceMemory(numElements); //Allocate device memory
    ...
    cudaError_t err = cudaDeviceSynchronize(); // Synchronize the device (wait for the kernel to finish)
    ...
    std::ofstream output_c_lock("output_c.lock");
    if (output_c_lock.is_open())
    {
        output_c_lock<<i;
        output_c_lock.close();
    }
    else
        std::cerr << "Failed to open the Lock file!" << std::endl;
